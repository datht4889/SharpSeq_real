...
class LInEx(MetaModule):
        ...
        if input_dim != hidden_dim:
            if dropout_type != "normal":
                ...
                self.input_map = CustomMetaSequential(OrderedDict({
                    #"dropout_int":dropout(input_dim, p=p, device=device, fixed=True if dropout_type != "adap" else False),
    ...
    def label_loss(self, label, labels, features, tau):
        ...
        #pfeatures = pfeatures/(torch.sum(pfeatures**2, dim=-1, keepdims=True)+1e-5)
        nfeatures = features[nidx, :]
        #nfeatures = nfeatures/(torch.sum(nfeatures**2, dim=-1, keepdims=True)+1e-5)
        sims = torch.sum(torch.exp(torch.matmul(pfeatures, nfeatures.T)/tau), dim=-1)
        if label != 0:
            pidx = pidx[::-1]
        rpfeatures = features[pidx, :]
        #rpfeatures = rpfeatures/(torch.sum(rpfeatures**2, dim=-1, keepdims=True)+1e-5)
    ...
    def forward(self, batch, nslots: int = -1, contrastive:bool=False, return_loss_list: bool=False, generate: bool = False, sample_size: int = 5, exemplar: bool = False,
        ...
        if return_loss:
            ...
            try:
                """
                loss_list.append((loss+lambda_coef*self.compute_KL_bernoulli(self.input_map[0].log_alpha)*rate 
                                 + lambda_coef*self.compute_KL_bernoulli(self.input_map[3].log_alpha)*rate)
            ...
            if exemplar and self.exemplar_features is not None:
                ...
                """if lambda_coef is not None:
                    try:
                        ...
                        alpha = alpha.repeat(exemplar_features.size(0), 1)
                        alpha = torch.sigmoid(alpha)
                        noise = torch.randn(alpha.size()).to(self.device)
                        exemplar_features = exemplar_features.to(self.device)
                        exemplar_features = exemplar_features*(1+noise*alpha)
                ...
                if self.is_adap:
                    ...
                    #loss_exemplar += lambda_coef*self.compute_KL_bernoulli(self.input_map[3].log_alpha)*rate
